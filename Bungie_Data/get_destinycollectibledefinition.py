...
def get_bungie_data():
    ...
    # Extract DestinyCollectibleDefinition from the aggregate data
    if 'DestinyCollectibleDefinition' not in aggregate_data:
        raise ValueError(f"DestinyCollectibleDefinition not found in aggregate data. Available keys: {list(aggregate_data.keys())}")
    collectible_data = aggregate_data['DestinyCollectibleDefinition']
    
    # Save the DestinyCollectibleDefinition data to a fixed filename
    output_file = 'DestinyCollectibleDefinition.json'
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(collectible_data, f, indent=4)
    
    print(f"DestinyCollectibleDefinition data saved to: {output_file}")
